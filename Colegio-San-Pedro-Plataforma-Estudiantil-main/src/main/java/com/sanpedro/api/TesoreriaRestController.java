...
public class TesoreriaRestController {
    ...
    public ResponseEntity<?> listarMiEstadoCuenta(Principal principal) {
        try {
            List<PagoPension> cronograma = tesoreriaService.obtenerEstadoCuentaMiAlumno(principal.getName());

            if (cronograma.isEmpty()) {
                return new ResponseEntity<>("No se encontraron pagos para tu matricula.", HttpStatus.NOT_FOUND);
            }

            List<PagoPensionResponseDto> responseList = cronograma.stream()
            ...
            return new ResponseEntity<>("Error al obtener tu estado de cuenta: " + e.getMessage(),
                    HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
